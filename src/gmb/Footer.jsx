...
const Footer = ({ onNavClick }) => {
  ...
  const navLinks = {
    "Quick Links": [
      { name: "Home", id: "home" },
      { name: "About GBP", id: "whatIsGBP" },
      { name: "Why It Matters", id: "whyGBP" },
      { name: "Why Choose Us", id: "whyChoose" },
      { name: "Services", id: "packages" },
      { name: "FAQ", id: "faqSection" },
    ],
    Legal: [
  ...
  // Handle navigation - either using the provided onNavClick function or fallback to scrolling
  const handleNavigation = (id, isExternalLink = false, href = null) => {
    ...
    if (onNavClick) {
      ...
      // Fallback navigation if onNavClick not provided
  ...
  return (
    <footer id="contact" className="relative overflow-hidden bg-[#09122C]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          <div className="col-span-1 md:col-span-2 lg:col-span-1">
            <div className="space-y-8">
              <div className="space-y-6">
                ...
                <p className="text-sm text-[#ECAF41]/80 transition-colors duration-300 max-w-sm leading-relaxed">
          ...
          {Object.entries(navLinks).map(([category, links]) => (
            <div key={category} className="space-y-6">
              <h3 className="text-sm font-semibold text-[#ECAF41] tracking-wider uppercase after:content-[''] after:block after:w-12 after:h-0.5 after:bg-[#ECAF41] after:mt-2">
                {category}
              </h3>
                ...
                {links.map((link) => (
                  <li key={link.name}>
                    <button
                      onClick={() =>
                        link.href
                          ? handleNavigation(null, true, link.href)
                          : handleNavigation(link.id)
                      }
                      className="group flex items-center text-sm text-[#ECAF41]/80 hover:text-[#ECAF41] transition-colors duration-300"
                    >
                      <ChevronRight className="h-4 w-4 mr-2 transform group-hover:translate-x-1 transition-transform opacity-0 group-hover:opacity-100 text-[#ECAF41]" />
                      {link.name}
                    </button>
                  </li>
                ))}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-4 text-[#ECAF41]/80 group">
                <MapPin className="h-5 w-5 text-[#ECAF41] flex-shrink-0 mt-1 group-hover:scale-110 transition-transform" />
              ...
              {contactInfo.map(({ Icon, text, href }, index) => (
                <a
                  key={index}
                  href={href}
                  className="flex items-center gap-4 text-[#ECAF41]/80 hover:text-[#ECAF41] group transition-colors duration-300"
                >
                  <Icon className="h-5 w-5 text-[#ECAF41] group-hover:scale-110 transition-transform" />
